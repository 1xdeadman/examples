...
Класс - структура данных, формальное описание того, так объект должен хранить и обрабатывать данные.
Объект класса - отдельный объект, который следует правилам класса.
Например: класс - это общее представление компьютера, а объект класса - это конкретно ваш компьютер.

В python функции, находящиеся в классе, называются методами, а переменные, так же входящие в класс - атрибутами.

Мы можем создать для класса:
 - собственные методы,
 - атрибуты,
 - переопределить операнды, н-р =,+,-,*,/, чтобы сообщить интерпретатору, как ему следует выполнять подобные операции с объектами класса.
"""
