...
Мы можем определить специальные методы, которые будут вызываться каждый раз при создании/удалении объекта.
...
class ConstructionCls:
    def __init__(self):
        ...
        def __init__(self, param_1, random_param_1):
            pass
        но тогда при создании объекта необходимо передавать аргументы в его конструктор:
    ...
    def __del__(self):
        ...
        Объект класса уничтожается в тот момент, когда объект класса более недостижим из кода программы
        """
        print("----:", "destructor")
